...
class Archelper(object):
    ...
    @print_timing        
    def batch_extract_by_mask(self, inputws, outputws, mask, extent='whole', suffix=''):
        ...
        try:
            ...
            while raster:
                if raster is not os.path.basename(mask):
                    ...
                    outRaster = '%s%s' % (os.path.join(outputws, os.path.basename(raster)), suffix)
                    print 'Extracting %s by mask...' % raster
    ...
    @print_timing        
    def batch_fix_nulldata(self, sourceWorkspace, targetWorkspace, ND='-1', ext='GRID'):
        try:
            self.gp.workspace = sourceWorkspace
            self.gp.toolbox = "SP"
    ...
    @print_timing
    def batch_GRID_to_ASCII(self, wdir, outWorkspace):
        ...
        try:
            ...
            while raster:
                try:
                    ...
                    outAsciiFile = '%s.asc' % os.path.basename(raster)
    ...
    def batch_multiply_rasters(self, sourceWorkspace, targetWorkspace, multiplier, ext='GRID'):
        try:
            ...
            raster = rasters.next()
            while raster:
                sys.stdout.write('Multiplying %s with %s...' % (raster, os.path.basename(multiplier)))
                raster = os.path.join(sourceWorkspace, raster)
                outRaster = os.path.join(targetWorkspace, os.path.basename(raster))
        ...
        try:
            ...
            while raster:
                if include != [] and raster not in include:
                    ...
                    # Version 1: create raster witn nodata = -1
                    # Version 2: create raster witn nodata = NoData
                    if not ndmode:
...
if __name__ == '__main__':
    ...
    #                          extent='metso')   
    #arc.batch_extract_by_mask('E:\Data\Metla\AnalysisLayers\Target\Fin\Metsatalousmaa\sr_kpsp', 
    #                          'E:\Data\Metla\AnalysisLayers\Target\Metso\Metsatalousmaa\sr_kpsp', 
    #                          'E:\Data\Metso\MetsoII.gdb\Alue\Metso_extent',
    #                         extent='metso')'''
    #arc.batch_GRID_to_ASCII(r'E:\Data\Metsahallitus\AnalysisLayers\Target300\Metso',     
    ...
    arc.set_nodata_aggregation(r'E:\Data\Metsahallitus\AnalysisLayers\Source\Fin', 
                               r'E:\Data\Metsahallitus\AnalysisLayers\Target300\Fin',
                               cellfactor=12,
                               ndmode=True,
                               extent=extent,
                               #mask=mask,
    ...
    #                     -9999, -1)
    #arc.batch_fix_nulldata(r'E:\Data\Metsahallitus\AnalysisLayers\Target\Metso_nd', 
    #                       r'E:\Data\Metsahallitus\AnalysisLayers\Target\Metso',
    #                       ND=0)
    ...
    #                      r'E:\Data\Metsahallitus\AnalysisLayers\Target\Fin\Metsatalousmaa\Biotooppi500\nat_kooste')
    #arc.batch_change_nodata(r'E:\Data\Metsahallitus\AnalysisLayers\Target\Fin\Metsatalousmaa\Biotooppi500\nat_kooste', 
    #                        r'E:\Data\Metsahallitus\AnalysisLayers\Target\Fin\Metsatalousmaa\Biotooppi500\nat_kooste\sub', 
    #                        oldND='0',
    #                        newND='-1')
